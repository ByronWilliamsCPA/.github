...
| `parallel-tests` | boolean | `false` | Opt-in: run unit/integration/security as parallel jobs (`test-unit`, `test-integration`, `test-security`) instead of sequential steps in `quality-checks`, cutting wall-clock time on repos with a large integration suite. A `coverage-combine` job merges their coverage data back into the same `coverage-reports` artifact, so no caller-side change is needed to adopt it. The `coverage-threshold` gate is enforced once by `coverage-combine` after the merge; each per-bucket run passes `--cov-fail-under=0` so a whole-suite threshold in the caller's `pyproject.toml` addopts does not false-fail a single bucket. **Requires** the caller's `pyproject.toml` to set `[tool.coverage.run] relative_files = true`; without it, `coverage combine` cannot correctly merge data recorded on three separate runners and the combined coverage percentage will be wrong. |
