...
- `supply-chain-build-verify.yml`: add `actions/upload-artifact` step and `artifact_name` output so the built `image.tar` is accessible to `supply-chain-promote-core` running on a separate runner. Without this step, passing `source_kind: oci-tar` to promote-core silently finds an empty artifact download because the tar existed only on the build runner's local filesystem. Fix header comment to show the correct caller wiring (`source_kind: oci-tar` + `source_artifact: ${{ needs.build.outputs.artifact_name }}`). Change workflow-level `permissions: contents: read` to `permissions: {}` (jobs that need permissions set them explicitly). Add fail-closed uv-installer checksum gate with a `UV_INSTALLER_SHA256` env variable that must be filled in before callers are wired (fails with a clear error if empty).
- `supply-chain-promote-core.yml`: fix Snyk token gate: `if: env.SNYK_TOKEN != ''` evaluated step-level env which GHA never exposes to `if:` expressions, so Snyk never ran. Replace with a dedicated `check-snyk-token` step that writes `has-token` to step outputs and gate the scan on `steps.check-snyk-token.outputs.has-token == 'true'`. Pin Snyk CLI to a specific version and remove `|| true` output suppression. Add masking for `AUTH_B64` and `AUTH_HEADER` (the GHA log scrubber masks exact strings only; base64 derivatives of a masked secret are not redacted automatically). Add `source_ref` digest shape-guard before `crane pull` to enforce the digest-pinned trust model. Add lock_file path traversal guard. Change workflow-level `permissions: contents: read` to `permissions: {}`. Add `#CRITICAL` RAD marker on the Trivy re-introduction (Trivy was removed in PR #229 due to confirmed supply-chain compromise; the re-introduction requires ADR-012 and documented re-vetting before merge).
- `supply-chain-consume-verify.yml`: propagate lock membership failure to the `verified` output when `enforce: false` (WARN mode). Previously, a digest not in the approved lock wrote only a `::warning::` and exited 0; the subsequent cosign step could then write `verified=true`, masking the failure. Now the lock-check step writes `lock_passed` to its step outputs and the verify step demotes `verified` to `false` when lock failed even if cosign passed. Fix `expected_issuer_regexp` default to anchored and dot-escaped form (`'^https://token\.actions\.githubusercontent\.com$'`); the unescaped dot matched any character.
- `supply-chain-mirror-verify.yml`: fix semantic inversion in `verified` output: when `require_upstream_signature=false`, the output contract documents "true when verified OR verification was not required," but the code wrote `verified=false`. Change to `verified=true`. Add shape-guards for `upstream_registry` and `upstream_name` before writing to `GITHUB_OUTPUT` to prevent newline injection (GITHUB_OUTPUT injection is a literal key=value substitution; a newline in a caller-controlled input injects extra output variables). Fix `expected_issuer_regexp` default to anchored and dot-escaped form. Change workflow-level `permissions: contents: read` to `permissions: {}`.
- `supply-chain-build-verify.yml`, `supply-chain-mirror-verify.yml`, `supply-chain-promote-core.yml`: add `--proto '=https'` to all `curl --location` invocations (SonarCloud S6506). Without this flag, `--location` follows redirects to any protocol; `--proto '=https'` restricts redirects to HTTPS only, preventing protocol-downgrade attacks during tool downloads (uv installer, crane tarball).
- `supply-chain-promote-core.yml`: add `--ignore-scripts` to `npm install -g snyk` (SonarCloud S6505). Without this flag, npm executes arbitrary lifecycle hooks (preinstall, postinstall, prepare) during package installation; `--ignore-scripts` disables all lifecycle scripts.
...
- `workflow-templates/python-codecov.yml`: rename the `file:` input to `files:` in all five per-version Codecov upload steps; `codecov/codecov-action` declares only `files:` (the singular `file:` is undeclared and silently ignored by GitHub Actions), so the named coverage XML was never passed to the uploader and uploads relied on the CLI's auto-discovery; surfaced during the v7 input-compatibility review on PR #199
