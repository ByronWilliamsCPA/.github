...
- `python-precommit.yml`: new reusable workflow that runs `pre-commit run --all-files` in the project virtualenv via `uv run`; inputs `config-path`, `python-version`, `show-diff-on-failure`; all inputs via env vars; SHA-pinned actions
