...
- `python-security-analysis.yml`: change `security-gate` job `harden-runner` `egress-policy` from `audit` to `block`; the `security-gate` job is a pure fan-in aggregator that consumes upstream job results via `needs.*.result` env vars and emits a pass/fail message with no outbound network calls, so blocking egress is the correct least-privilege setting at the runner level; the other jobs (`codeql`, `dependency-review`, `python-security`, `osv-scanner`) retain `egress-policy: audit` because they perform legitimate network operations (dependency fetch, CodeQL upload, OSV database queries); fixes #116
