...
- `python-release.yml`: run `cyclonedx-py` through `uv run` in the Generate SBOM step. `uv pip install` places `cyclonedx-bom` in the project venv, but the bare `cyclonedx-py` invocation does not have `.venv/bin` on PATH, so SBOM generation failed with exit 127 (command not found) in callers; first observed in Unify's `workflow_run` release. `uv run` resolves the venv entry point.
