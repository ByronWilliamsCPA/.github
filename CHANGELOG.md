...
- `scripts/fleet-audit-sha-pins.sh`: distinguish legitimate HTTP 404 (no workflows directory) from real API failures (rate limit, 5xx, auth loss); real failures now emit a `repo,error` sentinel row and a stderr WARN line instead of silently appearing as `repo,0`; bump `gh repo list --limit` from 200 to 1000 and warn when the result saturates the limit; replace regex-based skip-owners check with a literal `case` match so operator-supplied values containing regex metacharacters cannot produce unintended matches; addresses the silent-failure and regex-injection findings from the PR #175 follow-up review
- `scripts/update-pinned-actions.sh`: `extract_tag_pins` and `extract_branch_pins` now accept refs that carry an inline `# comment` (e.g., `actions/checkout@v4  # stable`); the converter previously required the ref to be the last non-whitespace content on the line, leaving inline-annotated refs as lingering audit violations; the sed substitution in `--pin-tags --apply` now spans to end-of-line so any pre-existing inline comment is replaced cleanly by the new `# <tag>` comment; escape sed-replacement metacharacters (`\`, `&`, `|`) in the resolved tag name before splicing into the substitution; hoist the `CHANGE_LOG` temp file and its cleanup trap to script scope so `pin_tags_main` no longer relies on a function-local `RETURN` trap; both extractor pipelines now tolerate zero matches under `set -euo pipefail`
- `tests/update-pinned-actions.bats`, `tests/fleet-audit-sha-pins.bats`: add five new test cases: refs with pre-existing inline comments are converted (and the old comment dropped), `--pin-tags` honors a custom `--owner-allowlist`, `--pin-tags` resolves annotated tags via two-step dereferencing, audit emits the `repo,error` sentinel when `gh api` returns a non-404 failure, and audit treats 404 on the workflows directory as a legitimate zero
